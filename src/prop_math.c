...
int v_print7(double v_to_print[7], char name[256]) {
    printf("%s: (%.7f, %.7f, %.7f) (%.7f, %.7f, %.7f) %.7f\n", name,
           v_to_print[0], v_to_print[1], v_to_print[2], v_to_print[3],
           v_to_print[4], v_to_print[5], v_to_print[6]);
    return 0;
}
...
int m_x_v6(double v_out[6], double **m_in, double v_in[6]) {
    ...
    v_out[2] = m_in[2][0] * v_in[0] + m_in[2][1] * v_in[1] + m_in[2][2] * v_in[2] +
               ...
               + m_in[2][4] * v_in[4] +
               m_in[2][5] * v_in[5];
    v_out[3] = m_in[3][0] * v_in[0] + m_in[3][1] * v_in[1] + m_in[3][2] * v_in[2] +
               m_in[3][3] * v_in[3]
               + m_in[3][4] * v_in[4] +
               m_in[3][5] * v_in[5];
    v_out[4] = m_in[4][0] * v_in[0] + m_in[4][1] * v_in[1] + m_in[4][2] * v_in[2] +
               m_in[4][3] * v_in[3]
               + m_in[4][4] * v_in[4] +
               m_in[4][5] * v_in[5];
    v_out[5] = m_in[5][0] * v_in[0] + m_in[5][1] * v_in[1] + m_in[5][2] * v_in[2] +
               m_in[5][3] * v_in[3]
               + m_in[5][4] * v_in[4] +
               m_in[5][5] * v_in[5];
    return 0;
...
int cart_to_equin(double *af, double *ag, double *lam, double *n, double *chi,
    ...
    *psi = -w[1] / (1 + fr * w[2]);
    ...
    double sinf = (*ag) + ((1 - (*ag) * (*ag) * b) * y - (*ag) * (*af) * b * x) /
                  (a * sqrt(1 -
                            (*ag) * (*ag) - (*af) * (*af)));
    double cosf = (*af) + ((1 - (*af) * (*af) * b) * x - (*ag) * (*af) * b * y) /
                  (a * sqrt(1 -
                            (*ag) * (*ag) - (*af) * (*af)));
...
int equin_to_cart(double rvec[3], double vvec[3], double af, double ag,
    ...
    double sinl = ((1 - af * af * b) * sin(F) + ag * af * b * cos(F) - ag) /
                  (1 - ag * sin(
                       F) - af * cos(F));
...
equin_to_cart(double T_equin_to_cart[6][6], double af, double ag, double lam,
    ...
    double r3 = r * r * r;
    double a3 = a * a * a;
    double dxdotdaf = a * xdot * ydot / (A * B) - A / r3 * (a * ag * x /
                      (1 + B) + x * y / B);
    double dydotdaf = -a * xdot * xdot / (A * B) - A / r3 * (a * ag * y /
                      (1 + B) - x * x / B);
    double dxdotdag = a * ydot * ydot / (A * B) + A / r3 * (a * af * x /
                      (1 + B) - y * y / B);
    ...
    T_equin_to_cart[3][3] = 2 * (fr * psi * (ydot * fe - xdot * ge) - xdot * we) /
                            C;
    T_equin_to_cart[3][4] = 2 * fr * (chi * (xdot * ge - ydot * fe) + ydot * we) /
                            C;
    ...
    T_equin_to_cart[4][3] = 2 * (fr * psi * (ydot * fq - xdot * gq) - xdot * wq) /
                            C;
    T_equin_to_cart[4][4] = 2 * fr * (chi * (xdot * gq - ydot * fq) + ydot * wq) /
                            C;
    ...
    T_equin_to_cart[5][3] = 2 * (fr * psi * (ydot * fw - xdot * gw) - xdot * ww) /
                            C;
    T_equin_to_cart[5][4] = 2 * fr * (chi * (xdot * gw - ydot * fw) + ydot * ww) /
                            C;
...
int compute_T_deriv_class_to_cart(double T_class_to_cart[6][6], double a,
    ...
    double c2 = -(2 * a * e + a * cos(nu) + a * e * e * cos(nu)) / ((1 + e * cos(
                    nu)) * (1 + e * cos(
                                nu)));
    double c3 = c1 / (1 + e * cos(nu));
    ri[0] = c1 * (cos(nu) * (cos(big_omega) * cos(small_omega) - sin(
                                 big_omega) * sin(
                                 small_omega) * cos(i)) - sin(nu) * (cos(big_omega) * sin(small_omega) +
                                         sin(big_omega) * cos(small_omega) * cos(i)));
    ri[1] = c2 * (cos(nu) * (cos(big_omega) * cos(small_omega) - sin(
                                 big_omega) * sin(
                                 small_omega) * cos(i)) - sin(
                      nu) * (cos(big_omega) * sin(small_omega) + sin(big_omega) * cos(
                                 small_omega) * cos(i)));
    ri[2] = a * c1 * sin(big_omega) * sin(i) * (cos(nu) * sin(small_omega) + sin(
                nu) * cos(
                small_omega));
    ri[3] = a * c1 * (-cos(nu) * sin(big_omega) * cos(small_omega) - cos(nu) * cos(
                          big_omega) * sin(
                          small_omega) * cos(i) + sin(nu) * sin(big_omega) * sin(small_omega) - sin(
                          nu) * cos(
                          big_omega) * cos(small_omega) * cos(i));
    ri[4] = a * c1 * (-cos(nu) * cos(big_omega) * sin(small_omega) - cos(nu) * sin(
                          big_omega) * cos(
                          small_omega) * cos(i) - sin(nu) * cos(big_omega) * cos(small_omega) + sin(
                          nu) * sin(
                          big_omega) * sin(small_omega) * cos(i));
    ri[5] = a * c3 * (-sin(nu) * (cos(big_omega) * cos(small_omega) - sin(
                                      big_omega) * sin(
                                      small_omega) * cos(i)) +
                      (e + cos(nu)) * (-cos(big_omega) * sin(small_omega) - sin(big_omega) * cos(
                                           small_omega) * cos(i)));
    ...
    rj[0] = c1 * (cos(nu) * sin(big_omega) * cos(small_omega) + cos(nu) * cos(
                      ...
                      i) - sin(nu) * sin(big_omega) * sin(small_omega) + sin(nu) * cos(
                      big_omega) * cos(
                      small_omega) * cos(i));
    rj[1] = c2 * (cos(nu) * (sin(big_omega) * cos(small_omega) + cos(
                                 big_omega) * sin(
                                 small_omega) * cos(i)) + sin(
                      nu) * (-sin(big_omega) * sin(small_omega) + cos(big_omega) * cos(
                                 small_omega) * cos(i)));
    rj[2] = -a * c1 * cos(big_omega) * sin(i) * (cos(nu) * sin(small_omega) + sin(
                nu) * cos(
                small_omega));
    rj[3] = a * c1 * (cos(nu) * cos(big_omega) * cos(small_omega) - cos(nu) * sin(
                          big_omega) * sin(
                          small_omega) * cos(i) - sin(nu) * cos(big_omega) * sin(small_omega) - sin(
                          nu) * sin(
                          big_omega) * cos(small_omega) * cos(i));
    ...
    rj[4] = a * c1 * (-cos(nu) * sin(big_omega) * sin(small_omega) + cos(nu) * cos(
                          big_omega) * cos(
                          small_omega) * cos(i) - sin(nu) * sin(big_omega) * cos(small_omega) - sin(
                          nu) * cos(
                          big_omega) * sin(small_omega) * cos(i));
    rj[5] = a * c3 * (-sin(nu) * (sin(big_omega) * cos(small_omega) + cos(
                                      big_omega) * sin(
                                      small_omega) * cos(i)) +
                      (e + cos(nu)) * (-sin(big_omega) * sin(small_omega) + cos(big_omega) * cos(
                                           small_omega) * cos(i)));

    double rk[6];
    //it's not rk, just a notation
    rk[0] = c1 * (cos(nu) * sin(small_omega) * sin(i) + sin(nu) * cos(
                      small_omega) * sin(
                      i));
    rk[1] = c2 * (cos(nu) * sin(small_omega) * sin(i) + sin(nu) * cos(
                      small_omega) * sin(
                      i));
    rk[2] = a * c1 * cos(i) * (cos(nu) * sin(small_omega) + sin(nu) * cos(
                                   small_omega));
    rk[3] = 0;
    rk[4] = a * c1 * sin(i) * (cos(nu) * cos(small_omega) - sin(nu) * sin(
                                   small_omega));
    ...
    vi[0] = 1 / (2 * a) * c3 * (sin(nu) * (cos(big_omega) * cos(small_omega) - sin(
            big_omega) * sin(
                    small_omega) * cos(i)) - (e + cos(nu)) * (-cos(big_omega) * sin(
                                    small_omega) - sin(big_omega) * cos(
                                    small_omega) * cos(i)));
    //!!!!looks like there s a typo in the equation online cos((w * cos(i)))
    vi[1] = 1 / (1 - e * e) * c3 * (-e * sin(nu) * (cos(big_omega) * cos(
                                        small_omega) - sin(
                                        big_omega) * sin(
                                        small_omega) * cos(i)) + (1 + e * cos(nu)) * (-cos(big_omega) * sin(
                                                small_omega) - sin(
    ...
    vi[3] = c3 * (sin(nu) * sin(big_omega) * cos(small_omega) + sin(nu) * cos(
                      big_omega) * sin(small_omega) * cos(
                      i) + (e + cos(nu)) * (sin(big_omega) * sin(small_omega) - cos(big_omega) * cos(
                              small_omega) * cos(
                                      i)));
    vi[4] = c3 * (sin(nu) * cos(big_omega) * sin(small_omega) + sin(nu) * sin(
                      big_omega) * cos(small_omega) * cos(
                      i) + (e + cos(nu)) * (-cos(big_omega) * cos(small_omega) + sin(big_omega) * sin(
                              small_omega) * cos(
                                      i)));
    vi[5] = c3 * (-cos(nu) * (cos(big_omega) * cos(small_omega) - sin(
                                  big_omega) * sin(
                                  small_omega) * cos(i)) - sin(
                      nu) * (-cos(big_omega) * sin(small_omega) - sin(big_omega) * cos(
                                 small_omega) * cos(i)));
    ...
    vj[0] = 1 / (2 * a) * c3 * (sin(nu) * (sin(big_omega) * cos(small_omega) + cos(
            big_omega) * sin(
                    small_omega) * cos(i)) - (e + cos(nu)) * (-sin(big_omega) * sin(
                                    small_omega) + cos(big_omega) * cos(
                                    small_omega) * cos(i)));
    //!!!!looks like there s a typo in the equation online cos((w * cos(i)))
    vj[1] = 1 / (1 - e * e) * c3 * (-e * sin(nu) * (sin(big_omega) * cos(
                                        small_omega) + cos(
                                        big_omega) * sin(
                                        small_omega) * cos(i)) + (1 + e * cos(nu)) * (-sin(big_omega) * sin(
                                                small_omega) + cos(
                                                        big_omega) * cos(small_omega) * cos(
                                                                i)));
    //!!!!looks like there s a typo in the equation online cos((w * cos(i)))
    ...
    vj[3] = c3 * (-sin(nu) * cos(big_omega) * cos(small_omega) + sin(nu) * sin(
                      big_omega) * sin(small_omega) * cos(
                      i) + (e + cos(nu)) * (-cos(big_omega) * sin(small_omega) - sin(big_omega) * cos(
                              small_omega) * cos(
                                      i)));
    vj[4] = c3 * (sin(nu) * sin(big_omega) * sin(small_omega) - sin(nu) * cos(
                      big_omega) * cos(small_omega) * cos(
                      i) + (e + cos(nu)) * (-sin(big_omega) * cos(small_omega) - cos(big_omega) * sin(
                              small_omega) * cos(
                                      i)));
    vj[5] = c3 * (-cos(nu) * (sin(big_omega) * cos(small_omega) + cos(
                                  big_omega) * sin(
                                  small_omega) * cos(i)) - sin(
                      nu) * (-sin(big_omega) * sin(small_omega) + cos(big_omega) * cos(
                                 small_omega) * cos(i)));
    ...
    vk[0] = 1 / (2 * a) * c3 * (sin(nu) * sin(small_omega) * sin(i) - (e + cos(
                                    nu)) * cos(
                                    small_omega) * sin(i));
    vk[1] = 1 / (1 - e * e) * c3 * (-e * sin(nu) * sin(small_omega) * sin(i) +
                                    (1 + e * cos(
                                         nu)) * cos(
                                        small_omega) * sin(i));
    //!!!!looks like there s a typo in the equation online cos((w * cos(i)))
    ...
    vk[4] = c3 * (-sin(nu) * cos(small_omega) * sin(i) - (e + cos(nu)) * (sin(
                      small_omega) * sin(i)));
    vk[5] = c3 * (-cos(nu) * sin(small_omega) * sin(i) - sin(nu) * cos(
                      small_omega) * sin(
                      i));
...
#swarm%2FLevel1b%2FEntire_mission_data%2FSTRxATT
    ...
    ///////////////////// NOT QUATERNION /////////////////////////////////////////////
        ...
        if(strcmp(attitude_profile, "sun_pointed") == 0) {
            ...
            double lvlh_z_in_lvlh[3], new_e_x_body_in_lvlh[3],
...
int compute_T_enu_to_ecef(double
                          T_enu_to_ecef[3][3], //out:rotation matrix ENU to ECEF
...
{
    /* Declarations */
    double T_enu_to_east_north_center_earth_to_local[3][3],
           T_east_north_center_earth_to_local_to_ecef[3][3];
    ...
    T_east_north_center_earth_to_local_to_ecef[0][2] = cos(geocentric_latitude) *
            cos(longitude);
    T_east_north_center_earth_to_local_to_ecef[1][0] = cos(longitude);
    T_east_north_center_earth_to_local_to_ecef[1][1] = -sin(
                geocentric_latitude) * sin(longitude);
    T_east_north_center_earth_to_local_to_ecef[1][2] = cos(geocentric_latitude) *
            sin(longitude);
